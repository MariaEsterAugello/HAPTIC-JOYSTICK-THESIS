...
#define S_FUNCTION_NAME  TimeSinc2
...
{   
	...
	double mult=mxGetScalar(ACCELERATOR_PARAM(S));
	
	int phase=ssGetIWorkValue(S, 0);
	if (phase==1) {
		//firts time called
		...
		if (phase==2) {
			...
			//phase==3 : RT sync
			sim_timer=ssGetT(S);
			deltatime=GetTickCount()-base_time;
			real_timer=((double)deltatime)/1000.0*mult;
